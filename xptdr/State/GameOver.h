...
#include "../UI/ClavierVirtuel.h"
#include "../UI/Scoreboard.h"
#include "../UI/Button.h"
#include"InterfaceState.h"

namespace StateNS {
	template <class MType> class GameOver : public State, public InterfaceState
	{
	private:
		bool askedScore;
		float score;
	...
	public:
		GameOver<MType>(GameDataRef _data) {
			data = _data;
			selectedButton = 0;
		}
		GameOver<MType>(GameDataRef _data, std::vector<Character> characters_, float scoreParam) {
			selectedButton = 0;
			data = _data;
	...
	void GameOver<MType>::STEInit()
	{
		...
		Shader.setUniform("u_resolution", sf::Glsl::Vec2(getEcranBound()));

		if (rank > 10) {
			//écrire le nom du looser
			updateFileRanks();
			scoreboard = Scoreboard(&(data->assets), rank);
		...
		textGameOver.setPosition((Engine::SCREEN_WIDTH / 2) - (textGameOver.getGlobalBounds().width / 2),
			(Engine::SCREEN_HEIGHT * 0.15f));

		Button button(&(data->assets));
		button.setString("Rejouer");
		button.setOutlineThickness(4);
		button.setThicknessColor(sf::Color::White);
		button.setSize(Engine::SCREEN_WIDTH * 0.15f, Engine::SCREEN_HEIGHT * 0.1f);
		button.setColor(sf::Color(174, 137, 100));
		button.setPosition((Engine::SCREEN_WIDTH * 0.2f-button.getGlobalBounds().width/2.f), 
			Engine::SCREEN_HEIGHT * 0.8f-button.getGlobalBounds().height/2.f);
		button.displayButtonInfo();
		buttonList.push_back(button);
		button.setString("Menu Principal");
		button.setOutlineThickness(0);
		button.setPosition(Engine::SCREEN_WIDTH * 0.8f - button.getGlobalBounds().width / 2.f,
			Engine::SCREEN_HEIGHT * 0.8f - button.getGlobalBounds().height / 2.f);
		buttonList.push_back(button);
		std::vector<UI*> mdrr;
		...
		//data->machine.AddState(StateRef(new Scoreboard(data, score)), false);
		data->assets.stopMusique("PartieJour");
	}
	template <class MType>
	void GameOver<MType>::STEHandleInput(sf::Event& event)
	{
	...
	template <class MType>
	void GameOver<MType>::STEUpdate(float delta)
	{
		if (rank <= 10 && !askedScore) {
			data->machine.AddState(StateRef(new ClavierVirtuel(data, score, rank, &playerName,(int)characters.size())), false);
			askedScore = true;
		}
		std::stringstream ss;
	...
	template <class MType>
	void GameOver<MType>::STEDraw(float delta)
	{
	...
	template<class MType>
	inline void GameOver<MType>::STEResume()
	{
		updateFileRanks();
	...
	template<class MType>
	inline void GameOver<MType>::updateCharacter()
	{
	...
	template<class MType>
	inline void GameOver<MType>::updateFileRanks()
	{
