	...
	: LaserGenerator()
{
	...
	bLaserAnim = CAnimation(&beginLaser, sf::Vector2i( (int)baseHeight, 400), sf::Vector2i(1,4), timeFrames);
	bLaserAnim.isHorizontal = false;
	fullLaser.setTexture(assets->GetTexture("fullLaser"));
	fLaserAnim = CAnimation(&fullLaser, sf::Vector2i(1824, 400), sf::Vector2i(1, 4), timeFrames);
...
{
	int screenheight = assets->sCREEN_HEIGHT;
...
float LaserGenerator::lengthToBound(sf::Vector2f& pos, float& angle)
{
	...
	float largeurEcran = (float)assets->sCREEN_WIDTH;
		float hauteurEcran = (float)assets->sCREEN_HEIGHT;
...
{
	float width = (float)assets->sCREEN_WIDTH; 
	float height = (float)assets->sCREEN_HEIGHT;
