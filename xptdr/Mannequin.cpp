...
void Mannequin::updateEtat(int& etat)
{
	...
	case 0:
		...
		anim = CAnimation(getPointerSprite(), sf::Vector2i(73, 79),4 , 0.3f, 5);
		break;
	case 1:
		onAvance = true;
		getSprite().setTexture(assets->GetTexture("micelsb"));
		anim = CAnimation(getPointerSprite(), sf::Vector2i(85, 260), 8, 0.3f, 12);
	...
	case 2 :
		...
		anim = CAnimation(getPointerSprite(), sf::Vector2i(87, 108), 2, 0.3f, 6);
...
void Mannequin::setAssets(CAssetManager* asset)
{
	assets = asset;
	etatmannequin = 0;
	updateEtat(etatmannequin);
	setPosition((float)assets->sCREEN_WIDTH * 0.75f, (float)assets->sCREEN_HEIGHT * 0.5f);
...
Mannequin::Mannequin(CAssetManager* asset_)
{
	setAssets(asset_);
}
...
void Mannequin::updatewPlayer(float delta, CPlayer& player)
{
