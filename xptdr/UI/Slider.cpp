...
Slider::Slider(float width, float length)
{
}

Slider::Slider(AssetManager* a, sf::Vector2f pos, sf::Vector2f size, std::string nameOfSlider)
	:Slider(a, size, nameOfSlider)
{
	setPosition(pos);
}

Slider::Slider(AssetManager* a, sf::Vector2f size, std::string nameOfSlider)
{
	value = 0;
	hasTitle = true;
	style = Horizontal;
...
void Slider::setName(std::string newName)
{
}

void Slider::setSize(float x, float y, int charSize)
{
...
void Slider::setCharacterSize(unsigned int charSize) {
	if (hasTitle)
	{
		NameOfSlider.setCharacterSize(charSize);
		textPercentage.setCharacterSize(charSize);
