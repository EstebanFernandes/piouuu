...
public:
	...
	void addCards(std::vector<Card>& cardList)
	{
		...
		for (int i = 0; i < nbEntity; i++)
		{
			...
			tempCard.sprite.anim = Animation(&tempCard.sprite.sprite, frame, 4, 0.16f);
	...
	void update(float deltaTime) {
		...
		if (isMoving) {
			...
			for (int i = 0; i < cards.size(); i++)
			{
				...
				if (carouselClock.getElapsedTime().asSeconds() > 0.0015f && nextindex != curIndex)
				{
