#include "ClavierVirtuel.h"

ClavierVirtuel::ClavierVirtuel(GameDataRef _data, float scoreParam, int rankParam, std::string *playerNameParam, int nbJoueur) : data(_data)
{
	score = scoreParam;
	rank = rankParam;
...
void ClavierVirtuel::keyboardInit()
{

}

void ClavierVirtuel::STEInit()
{
	...
	if (nbJoueur == 2)
	{
		...
		ah.setPosition(
			...
			(float)Engine::SCREEN_HEIGHT * 0.3f);
		...
		ah.setPosition(keyboardBlackSprite.getPosition().x + ((keyboardBlackSprite.getGlobalBounds().width - ah.getLocalBounds().width) * 0.5f), (float)Engine::SCREEN_HEIGHT * 0.3f);
		players[0].fondBlanc = ah;
		midText.setPosition(keyboardBlackSprite.getPosition().x + keyboardBlackSprite.getGlobalBounds().width / 2.f, ah.getPosition().y * 1.03f);
		midText.setCharacterSize(25);
...
void ClavierVirtuel::STEHandleInput(sf::Event& event)
{
...
void ClavierVirtuel::STEUpdate(float delta)
{
	updateTime();
...
void ClavierVirtuel::UpdateText(int joueur)
{
...
void ClavierVirtuel::STEDraw(float delta)
{
...
void ClavierVirtuel::updateBars(int index)
{
...
void ClavierVirtuel::setBar(int index)
{
