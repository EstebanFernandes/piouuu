...
Boss::Boss(AssetManager* assetsParam, Player* playerParam, std::vector<HittingEntity*>* entityListParam, int* enemyNumberParam)
{
	...
	rotate(180);
	setTexture("boss");
	spawnPos.x = ((float)Engine::SCREEN_WIDTH) * 1.15f;
	spawnPos.y = ((float)Engine::SCREEN_HEIGHT) * 0.5f;
...
void Boss::addEnemyColumn()
{
	...
	int enemyColumnNumber = Engine::SCREEN_HEIGHT / (int)enemyTest.getSprite().getGlobalBounds().height;
	...
	for (int i = 0; i < enemyColumnNumber; i++) {
		if (i != enemyMissing) {
			...
			enemy->setScoreGived(0);
			enemy->setLevel(0);
			enemy->setPositionEntity((float)Engine::SCREEN_WIDTH, enemyTest.getSprite().getGlobalBounds().height / 2 + Engine::SCREEN_HEIGHT * (i / (float)enemyColumnNumber));
			entityList->push_back(enemy);
			(*enemyNumber)++;
		}
	}
...
void Boss::updateMovement(float delta)
{
	...
	if (!isPositionated && getSprite().getPosition().x <= Engine::SCREEN_WIDTH - getSprite().getGlobalBounds().width / 2.f) {
		isPositionated = true;
	}
}
...
void Boss::updatewPlayer(float delta, Player& player)
{
