#include "Player.h"
#include"Weapon/chargeShot.h"



void Player::setSprite()
{
	setPositionEntity((Engine::SCREEN_WIDTH / 4.f) - (getSprite().getGlobalBounds().width / 2), (Engine::SCREEN_HEIGHT / 2.f) - (getSprite().getGlobalBounds().height / 2.f));
}
...
void Player::setValue(float& init, std::string modif)
{
	switch (modif[0])
	{
	case '*':
...
void Player::setValue(int& init, std::string modif)
{
	switch (modif[0])
	{
	case '*':
...
void Player::setAssets(AssetManager* a)
{
	
	assets = a;
	setMainWeapon(new Gunslinger(),true);
	setSecondaryWeapon(new Gunslinger(),true);
	setTexture(name);
	R2Sprite.setTexture(assets->GetTexture("R2D2"));
	R2Sprite.setOrigin(0.f, 0.f);
	R2Anim = Animation(&R2Sprite, sf::Vector2i( 30, 30), sf::Vector2i(1,3), 0.16f,2);
	R2Offset = sf::Vector2f(22.f, -6.f);
	if (isAnimated) anim = Animation(getPointerSprite(), sf::Vector2i( 153, 66), sf::Vector2i(4,3), 0.16f);
	std::string nameImage;
	WeaponStat ws((float)damagePerBullet, bulletSpeed, sf::Vector2f(1.f, 0.f), 0, nameImage, sf::Vector2f(1.f, 1.f), attackSpeed);
	...
	animExplosionSprite = Animation(&explosionSprite, sf::Vector2i( 96, 96), sf::Vector2i(12,1), 2.f/12.f);
	setSprite();
...
bool Player::checkGlobalCollisions()
{
	...
	else if(currentBounds.top + currentBounds.height > Engine::SCREEN_HEIGHT){
		setPositionEntity(nextBounds.left, Engine::SCREEN_HEIGHT - currentBounds.height / 2.f);
		currentBounds = getGlobalBounds();
		nextBounds = currentBounds;
		nextBounds.left += currentBounds.width / 2.f;
		nextBounds.top += currentBounds.height / 2.f;
	...
	else if (currentBounds.left + currentBounds.width > Engine::SCREEN_WIDTH){
		setPositionEntity(Engine::SCREEN_WIDTH - currentBounds.width / 2.f, nextBounds.top);
		currentBounds = getGlobalBounds();
		nextBounds = currentBounds;
		nextBounds.left += currentBounds.width / 2.f;
		nextBounds.top += currentBounds.height / 2.f;
...
void Player::setMainWeapon(Weapon* weaponParam,bool isFirstTime)
{
	setWeapon(&mainWeapon, weaponParam, isFirstTime);
}

void Player::setWeapon(Weapon** weaponToReplace, Weapon* replacement, bool isFirstTime)
{
	if (!isFirstTime&& *weaponToReplace != NULL)
	{
		...
		WeaponStat a = (*weaponToReplace)->getWeaponStats();
...
void Player::setSecondaryWeapon(Weapon* weaponParam, bool isFirstTime)
{
...
void Player::traitermisc(std::string& misc)
{
...
void Player::traitermisc(std::string& misc, int type)
{
...
"gunshot","dot", "laser","explosiveBullet","spin","doubleTirs3","fire","ice","thunder"};
	...
	if (pos >= supportedMisc.size()) {
		...
		if (type == 1)
		{
			...
			WeaponStat a = secondaryWeapon->getWeaponStats();
		...
		else if (type == 0)
		{
			...
			WeaponStat a = mainWeapon->getWeaponStats();
			setMainWeapon(new laserWeapon(assets));
...
void Player::initLifeBar()
{
	...
	lifeBar = sf::RectangleShape(sf::Vector2f(Engine::SCREEN_WIDTH / 8.f, 15.f));
...
void Player::PLYupdateMovement(sf::Event& event)
{
...
void Player::renderLifeBar(sf::RenderTarget& target)
{
	target.draw(xpBar);
...
bool Player::matchTypewithValue(std::string type, std::string value)
{
	return false;
}

void Player::setLifeBarPosition(sf::Vector2f& pos)
{
...
void Player::setNumero(int& i)
{
	...
	initLifeBar();
	sf::Vector2f pos(Engine::SCREEN_WIDTH * 0.05f, Engine::SCREEN_HEIGHT * 0.05f);
	switch (i)
	{
	case 1:
		...
		break;
	case 2:
		pos = sf::Vector2f(Engine::SCREEN_WIDTH * 0.95f - lifeBarBG2.getGlobalBounds().width-iconCircle2.getGlobalBounds().width, Engine::SCREEN_HEIGHT * 0.05f);
		setLifeBarPosition(pos);
		break;
	}
	numero = i;
...
void Player::reduceHP(float damage)
{
	Mob::reduceHP(damage);
}
