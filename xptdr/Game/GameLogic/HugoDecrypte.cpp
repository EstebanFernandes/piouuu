...
void CHugoDecrypte::VertexCreation(std::vector<std::string> motclefs, int k, int j, GrapheUpdate& G)
{
	SommetUpgrade temp;
	G.GRAAjouterSommet(temp);
	SommetUpgrade& currentSommet = G.GRAObtenirListeSommet()[G.GRAObtenirListeSommet().size()-1];
...
{
fileContent = IO::retrieveCSVFile(fileName);
parseFile();
}
