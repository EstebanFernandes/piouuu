...
#include"../State/InterfaceState.h"
#include<sys/stat.h>
...
{
	assets = NULL;
	graphs.push_back(std::vector<GrapheUpdate>());
	graphs[0].push_back(GrapheUpdate());
...
void upgradeStock::addGraphs(std::string fileName)
{
	...
	{
		SommetUpgrade tempSommet = temp.returnGraphs()[i].GRAObtenirListeSommet()[0];
...
void upgradeStock::avancer(graphComponent& comp, int next)
{
	...
	{
		GrapheUpdate& curGraph = graphs[comp.pos.y][comp.pos.x];
...
GrapheUpdate& upgradeStock::currentGraph(graphComponent g)
{
